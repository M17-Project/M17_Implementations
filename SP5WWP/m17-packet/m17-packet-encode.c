...
short empty[960*50];                                        //empty short samples to write to wav file as 'dead air'
...
FILE* fp;
float full_packet[1920*37];                                 //full packet, samples as floats - (40+40+32*40+40+40)/1000*4800
...
int main(int argc, char* argv[])
{
    ...
    if(argc>=4)
    {
        for(uint8_t i=1; i<argc; i++)
        {
            if(argv[i][0]=='-')
            {
                ...
                else if(argv[i][1]=='w') //-w - raw filtered output to wav file (rrc)
                {
                    out_type=4;
                }
    ...
    //fill preamble
    memset((uint8_t*)full_packet, 0.0f, 1920*37*sizeof(float));
    ...
    while(num_bytes)
    {
        ...
        {
            //debug num_bytes here
            // fprintf (stderr, " Last num: %d; ", num_bytes);

            memcpy(pkt_chunk, &full_packet_data[pkt_cnt*25], num_bytes);
            memset(&pkt_chunk[num_bytes], 0, 25-num_bytes); //zero-padding

            //subtract 2 for the CRC? or does the CRC count as significant bytes of the last block
            num_bytes -= 2;
    ...
    //initialize 'dead air' empty array as all zeroes
    memset (empty, 0, 960*50*sizeof(short));
    ...
    if(out_type==0)
    {
        //flush output file with ~1 second of empty 'dead air', helps with demodulation / decoding later on
        for (uint16_t i=0; i<960*50; i++)
            fwrite((uint8_t*)&empty[i], 2, 1, fp);
    ...
    {   
        //flush output file with ~1 second of empty 'dead air', helps with demodulation / decoding later on
        sf_write_short(wav, empty, 960*50);
